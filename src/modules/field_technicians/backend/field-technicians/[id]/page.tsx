...
function buildDisplayName(firstName: string | null, lastName: string | null, fallback: string): string {
  const displayName = [firstName ?? '', lastName ?? ''].map((part) => part.trim()).filter(Boolean).join(' ').trim()
...
export default function FieldTechnicianDetailPage({ params }: { params?: { id?: string } }) {
  ...
  const certFields = React.useMemo<CrudField[]>(() => [
    {
      id: 'name',
      label: t('fieldTechnicians.cert.form.name', 'Certificate name'),
      type: 'text',
      required: true,
      placeholder: t('fieldTechnicians.cert.form.namePlaceholder', 'e.g. SEP up to 1kV, category B driving license'),
    },
    {
      id: 'certType',
    ...
    {
      id: 'code',
      label: t('fieldTechnicians.cert.form.code', 'Certificate number'),
      type: 'text',
      placeholder: t('fieldTechnicians.cert.form.codePlaceholder', 'Certificate or permit number'),
    },
    ...
    {
      id: 'issuedBy',
      label: t('fieldTechnicians.cert.form.issuedBy', 'Issued by'),
      type: 'text',
      placeholder: t('fieldTechnicians.cert.form.issuedByPlaceholder', 'Issuing authority or institution'),
    },
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'edit' && (
            <CrudForm<TechFormValues>
              ...
              onSubmit={async (vals) => {
                const displayName = buildDisplayName(vals.firstName, vals.lastName, technician?.display_name ?? vals.displayName)
                await updateCrud('field-technicians', {
                  ...vals,
                  displayName,
                })
