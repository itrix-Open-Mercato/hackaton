...
export async function GET(req: Request) {
  try {
    ...
    const rows: { name: string }[] = await knex('technician_certifications')
      .select('name')
    ...
    console.error('machine_catalog.service_type_certifications.suggest failed', err)
    return NextResponse.json({ error: 'Failed to suggest certifications' }, { status: 500 })
  }
}

export const openApi = {
  summary: 'Autosuggest certification names from technician records',
