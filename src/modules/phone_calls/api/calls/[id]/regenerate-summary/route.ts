...
import { regenerateSummarySchema } from '../../../../data/validators'
...
export async function POST(req: Request, context: { params?: { id?: string } }) {
  try {
    ...
    const input = { ...regenerateSummarySchema.parse(body), id: callId }
    const commandBus = ctx.container.resolve('commandBus') as CommandBus
    const { result } = await commandBus.execute('phone_calls.summary.regenerate', { input, ctx })
    ...
    const message = err instanceof Error && err.message ? err.message : 'Failed to regenerate summary'
...
export const openApi: OpenApiRouteDoc = {
  tag: phoneCallsTag,
  summary: 'Regenerate phone call summary',
  methods: {
    POST: {
      summary: 'Regenerate summary from Tillio',
      requestBody: { contentType: 'application/json', schema: regenerateSummarySchema },
      responses: [
        { status: 200, description: 'Summary version created', schema: responseSchema },
