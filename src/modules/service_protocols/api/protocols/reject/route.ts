...
import { rejectSchema } from '../../../data/validators'
...
export async function POST(req: Request) {
  try {
    ...
    await commandBus.execute('service_protocols.protocols.reject', { input: parsed, ctx })
    ...
    console.error('service_protocols.protocols.reject failed', err)
...
export const openApi: OpenApiRouteDoc = {
  ...
  summary: 'Reject protocol',
  methods: {
    POST: {
      summary: 'Reject a protocol',
      description: 'Moves protocol from in_review back to draft with a rejection note.',
      requestBody: { contentType: 'application/json', schema: rejectSchema },
      responses: [{ status: 200, description: 'Protocol rejected', schema: z.object({ ok: z.boolean() }) }],
      errors: [
        { status: 400, description: 'Validation failed', schema: z.object({ error: z.string() }) },
        { status: 403, description: 'Coordinator permission required', schema: z.object({ error: z.string() }) },
      ],
    },
  },
}
