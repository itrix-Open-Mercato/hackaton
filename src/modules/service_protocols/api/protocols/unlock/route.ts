...
import { unlockSchema } from '../../../data/validators'
...
export async function POST(req: Request) {
  try {
    ...
    await commandBus.execute('service_protocols.protocols.unlock', { input: parsed, ctx })
    ...
    console.error('service_protocols.protocols.unlock failed', err)
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      summary: 'Unlock a closed protocol',
      description: 'Moves protocol from closed back to approved for corrections.',
      requestBody: { contentType: 'application/json', schema: unlockSchema },
      responses: [{ status: 200, description: 'Protocol unlocked', schema: z.object({ ok: z.boolean() }) }],
      errors: [{ status: 403, description: 'Coordinator permission required', schema: z.object({ error: z.string() }) }],
    },
  },
}
