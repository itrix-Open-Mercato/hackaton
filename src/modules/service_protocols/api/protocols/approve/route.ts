...
import { approveSchema } from '../../../data/validators'
...
export async function POST(req: Request) {
  try {
    ...
    await commandBus.execute('service_protocols.protocols.approve', { input: parsed, ctx })
    ...
    console.error('service_protocols.protocols.approve failed', err)
...
export const openApi: OpenApiRouteDoc = {
  ...
  methods: {
    POST: {
      summary: 'Approve a protocol',
      description: 'Moves protocol from in_review to approved.',
      requestBody: { contentType: 'application/json', schema: approveSchema },
      responses: [{ status: 200, description: 'Protocol approved', schema: z.object({ ok: z.boolean() }) }],
