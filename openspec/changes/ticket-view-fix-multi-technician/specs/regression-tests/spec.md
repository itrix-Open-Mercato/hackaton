...
### Requirement: Technician injection-table shape SHALL be tested

A test SHALL verify that the technicians module's `injection-table.ts` exports a valid `ModuleInjectionTable` record (object keyed by spot ID), not an array.

#### Scenario: Injection table is an object with spot keys
- **WHEN** the injection table is imported
- **THEN** it SHALL be a plain object (not an array)
- **AND** it SHALL contain keys for the registered injection spots

### Requirement: Technician create redirect SHALL be tested

A test SHALL verify that a successful technician creation navigates to the correct edit URL with the actual created ID.

#### Scenario: Create response ID used in redirect
- **WHEN** the create API returns `{ id: "<uuid>" }`
- **THEN** the navigation target SHALL be `/backend/technicians/<uuid>/edit`

### Requirement: Technician notes null preservation SHALL be tested

A test SHALL verify that clearing the notes field on the technician edit form results in `null` being sent to the API, not `undefined`.

#### Scenario: Empty notes string maps to null
- **WHEN** the edit form submit handler processes notes as `""`
- **THEN** the API payload SHALL include `notes: null` or `notes: ""`
- **AND** the value SHALL NOT be `undefined`

### Requirement: Service ticket edit page loading SHALL be tested

A test SHALL verify that the service ticket edit page correctly loads a ticket by ID without showing a "not found" error when the ticket exists.

#### Scenario: Successful ticket load
- **WHEN** `fetchCrudList` returns a valid ticket item for the given ID
- **THEN** the edit page SHALL set the initial form values from the ticket data
- **AND** no error state SHALL be set
